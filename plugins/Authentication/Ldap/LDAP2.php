...
class Net_LDAP2 extends PEAR
{
    ...
    protected function performConnect()
    {
        ...
        if (!is_array($this->_host_list) || 0 == count($this->_host_list)) {
        ...
        foreach ($this->_host_list as $host) {
            ...
            if (self::isError($msg)) {
                ...
                if (0x02 == $msg->getCode() && !$version_set) {
    ...
    public function add($entry)
    {
        ...
        while (true) {
            ...
            if (@ldap_add($link, $entry->dn(), $entry->getValues())) {
                ...
                if (('LDAP_OPERATIONS_ERROR' === $error_name)
     ...
     * @param bool                   $recursive Should we delete all children recursive as well?
     *
     * @return Net_LDAP2_Error|true Net_LDAP2_Error object or true
     */
    public function delete($dn, $recursive = false)
    {
        ...
        while (true) {
            ...
            if (@ldap_delete($link, $dn)) {
                ...
                if (('LDAP_OPERATIONS_ERROR' === Net_LDAP2::errorMessage($error_code))
                    && $this->_config['auto_reconnect']) {
                    // The server has become disconnected before trying the
    ...
    public function search($base = null, $filter = null, $params = [])
    {
        ...
        while (true) {
            ...
            if ($err = @ldap_errno($link)) {
                ...
                } elseif ((1 == $err) && $this->_config['auto_reconnect']) {
     ...
     * @param bool $force   If set to true, the check against the rootDSE will be skipped
     *
     * @return Net_LDAP2_Error|true Net_LDAP2_Error object or true
     *
    ...
    public function dnExists($dn)
    {
        ...
        return (0 == $search->count()) ? false : true;
    }
 ...
 * @license  http://www.gnu.org/copyleft/lesser.html LGPL
 *
 * @see     http://pear.php.net/package/Net_LDAP22/
 */
class Net_LDAP2_Error extends PEAR_Error
{
     ...
     * @param int    $code      Net_LDAP2 error code
