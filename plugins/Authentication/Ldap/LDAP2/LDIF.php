...
class Net_LDAP2_LDIF extends PEAR
{
     ...
     * </code>
     *
     * @param bool $as_string If set to true, only the message is returned
     *
     ...
     *
     * @param bool $force Set this to true if you want to iterate over the lines manually
     *
     * @return array
     */
    public function next_lines($force = false)
    {
        ...
        if (0 == count($this->_lines_next) || $force) {
    ...
    {
        // prepare DN
        if ('base64' == $this->_options['encode']) {
            $dn = $this->convertDN($dn).PHP_EOL;
        } elseif ('canonical' == $this->_options['encode']) {
    ...
    {
        if (is_resource($this->handle()) && false === fwrite($this->handle(), $line, strlen($line))) {
    ...
    protected function dropError($msg, $line = null)
    {
        ...
        if ('die' == $this->_options['onerror']) {
            exit($msg.PHP_EOL);
        } elseif ('warn' == $this->_options['onerror']) {
            echo $msg.PHP_EOL;
        }
    }
}
