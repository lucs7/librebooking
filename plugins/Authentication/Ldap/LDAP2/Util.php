...
class Net_LDAP2_Util extends PEAR
{
        ...
        foreach ($dn as $pos => $dnval) {
            if (is_array($dnval)) {
                ...
                if (count($rdns) > 1) {
                    ...
                    if ('oid.' == substr(strtolower($ocl), 0, 4)) {
                        $ocl = substr($ocl, 4);
                    } else {
                        if ('upper' == $options['casefold']) {
                            $ocl = strtoupper($ocl);
                        }
                        if ('lower' == $options['casefold']) {
                            $ocl = strtolower($ocl);
                        }
     ...
     * @param bool   $extended  If set to true, also filter-assertion delimeter will be matched
     * @param bool   $withDelim If set to true, the return array contains the delimeter at index 1, putting the value to index 2
     *
