...
class adLDAPExchange
{
    ...
    public function addAddress($username, $emailAddress, $default = false, $isGUID = false)
    {
        ...
        if (is_array($user[0]['proxyaddresses']) && true === $default) {
            $modAddresses = [];
            for ($i = 0; $i < sizeof($user[0]['proxyaddresses']); ++$i) {
                if (false !== strstr($user[0]['proxyaddresses'][$i], 'SMTP:')) {
    ...
    public function deleteAddress($username, $emailAddress, $isGUID = false)
    {
        ...
        if (is_array($user[0]['proxyaddresses'])) {
            $mod = [];
            for ($i = 0; $i < sizeof($user[0]['proxyaddresses']); ++$i) {
                if (false !== strstr($user[0]['proxyaddresses'][$i], 'SMTP:') && $user[0]['proxyaddresses'][$i] == 'SMTP:'.$emailAddress) {
                    $mod['proxyAddresses'][0] = 'SMTP:'.$emailAddress;
                } elseif (false !== strstr($user[0]['proxyaddresses'][$i], 'smtp:') && $user[0]['proxyaddresses'][$i] == 'smtp:'.$emailAddress) {
                    $mod['proxyAddresses'][0] = 'smtp:'.$emailAddress;
                }
            }
