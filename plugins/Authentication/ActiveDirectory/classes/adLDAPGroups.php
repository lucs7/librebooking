...
class adLDAPGroups
{
    ...
    public function inGroup($group, $recursive = null)
    {
        ...
        for ($i = 0; $i < $groups['count']; ++$i) {
            ...
            if (0 == $entries['count'] && true == $recursive) {
                $filter = '(&(objectCategory=group)(distinguishedName='.$this->adldap->utilities()->ldapSlashes($groups[$i]).'))';
    ...
    public function members($group, $recursive = null)
    {
        ...
        for ($i = 0; $i < $users['count']; ++$i) {
            ...
            if (0 == $entries['count'] && true == $recursive) {
                ...
                continue;
            } elseif (0 == $entries['count']) {
                continue;
            }

            if ((!isset($entries[0]['samaccountname'][0]) || null === $entries[0]['samaccountname'][0]) && null !== $entries[0]['distinguishedname'][0]) {
                $userArray[] = $entries[0]['distinguishedname'][0];
            } elseif (null !== $entries[0]['samaccountname'][0]) {
