...
class adLDAP
{
    ...
    {
        if (true === $useSSO && !$this->ldapSaslSupported()) {
    ...
    public function connect()
    {
        ...
        if (null !== $this->adminUsername && null !== $this->adminPassword) {
        ...
        if ($this->useSSO && $_SERVER['REMOTE_USER'] && null === $this->adminUsername && $_SERVER['KRB5CCNAME']) {
    ...
    public function authenticate($username, $password, $preventRebind = false)
    {
        ...
        if ($this->useSSO && $_SERVER['REMOTE_USER'] && $_SERVER['REMOTE_USER'] == $username && null === $this->adminUsername && $_SERVER['KRB5CCNAME']) {
            putenv('KRB5CCNAME='.$_SERVER['KRB5CCNAME']);
            $this->ldapBind = @ldap_sasl_bind($this->ldapConnection, null, null, 'GSSAPI');
            if (!$this->ldapBind) {
                throw new adLDAPException('Rebind to Active Directory failed. AD said: '.$this->getLastError());
        ...
        if (null !== $this->adminUsername && !$preventRebind) {
            $this->ldapBind = @ldap_bind($this->ldapConnection, $this->adminUsername.$this->accountSuffix, $this->adminPassword);
            if (!$this->ldapBind) {
