...
class CalendarSubscriptionServiceTest extends TestBase
{
    ...
    public function testGetResourceGroupNameDoesNotQueryDatabaseWhenGroupAlreadyLoadedByGetResourcesInGroup()
    {
        $publicId = uniqid();
        $group = new ResourceGroup(5, 'Engineering Rooms');

        $groupTree = $this->createMock('ResourceGroupTree');
        $groupTree->method('GetResourceIds')->willReturn([]);
        ...
        $name = $this->service->GetResourceGroupName($publicId);

        $this->assertEquals('Engineering Rooms', $name);
    }
